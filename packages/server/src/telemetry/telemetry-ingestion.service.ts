import type { IngestBatchResponse, TelemetryBatch } from '@blackbox/contracts';
...
export class TelemetryIngestionService {
  ...
  async ingest(ingestKey: string, batch: TelemetryBatch): Promise<IngestBatchResponse> {
    this.requireCurrentTimestamps(batch);
    if (!hasValidTelemetryEventSizes(batch)) {
      throw new BadRequestException('Telemetry events must be at most 32 KiB');
    }
  ...
  private requireCurrentTimestamps(batch: TelemetryBatch): void {
    const now = Date.now();
    ...
    if (
      ...
      throw new BadRequestException(
        'Telemetry timestamps must be within the accepted ingestion window',
      );
    }
  }
}

...
export class TelemetryQueueService implements OnModuleDestroy {
  ...
  private queue?: Queue<TelemetryJob>;
  ...
  async enqueue(job: TelemetryJob): Promise<void> {
    ...
    try {
      ...
      await queue.add('telemetry-batch', job, { jobId: job.batch.batchId });
    } catch (error) {
  ...
  private getQueue(): Queue<TelemetryJob> {
    if (this.queue) {
      return this.queue;
    }
    this.queue = new Queue<TelemetryJob>(TELEMETRY_QUEUE_NAME, {
